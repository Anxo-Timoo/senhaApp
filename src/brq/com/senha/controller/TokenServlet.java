...
public class TokenServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int sysubrc = 0;
		Random r = new Random();
		String usuarioSap = request.getParameter("loginSAP");
		String emailBRQ = request.getParameter("emailBRQ");	
		
		Usuario usuario = new Usuario();
		usuario.setLoginSap(usuarioSap);
		usuario.setEmail(emailBRQ);
		short randomNumber = (short)r.nextInt(Short.MAX_VALUE + 1);
		usuario.setSenha(String.valueOf(randomNumber));
		
		try {
			sysubrc = usuarioDao.registerUserSAP(usuario);			 
			result = true;
		} 
